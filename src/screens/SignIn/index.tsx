...
export function SignIn() {
  ...
  async function handleSignIn(){// creates a function to handle sign in
    try{
      await signIn();// try to call and wait signIn
    }catch(error){
      Alert.alert('Erro SignIn', 'Ocorreu um erro ao tentar logar no app');// if fails, display an Alert with the title "Erro SignIn" and message "Ocorreu um erro ao tentar logar no app"
    }
  }
      ...
      <Content>
        ...
        <LoginInfo>
          ...
          { <SignInButton onPress={handleSignIn}>
            ...
            <SignInButtonText>
              {isLoggingIn ? <Text>Entrando...</Text> : <Text>Entrar com Twitch</Text>}
            </SignInButtonText>
          </SignInButton> }
        </LoginInfo>
      </Content>
